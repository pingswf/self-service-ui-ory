...
func main() {
	...
	// start server
	log.Println("Auth Server listening on port 4485")
...
func NewServer(kratosPublicEndpointPort, hydraPublicEndpointPort, hydraAdminEndpointPort int) (*server, error) {
	...
	conf.Servers = ory.ServerConfigurations{{URL: fmt.Sprintf("http://kratos-public:%d", kratosPublicEndpointPort)}}
	...
	hydraConf.Servers = ory.ServerConfigurations{{URL: fmt.Sprintf("http://hydra-admin:%d", hydraAdminEndpointPort)}}
	...
	oauth2Conf := &oauth2.Config{
		...
		Endpoint: oauth2.Endpoint{
			...
			TokenURL: fmt.Sprintf("http://hydra-public:%d/oauth2/token", hydraPublicEndpointPort),    // access from server
		},
